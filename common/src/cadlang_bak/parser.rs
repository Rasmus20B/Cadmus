...
use super::{
    cadscript::{compile_cadscript, proto_script::*},
    token::{Token, TokenType},
};
use super::{error::CompileErr, staging::*};
use crate::dbobjects::{
    data_source::*,
    layout::*,
    ...
    schema::{
        ...
        relationgraph::{relation::*, table_occurrence::TableOccurrence},
    },
};
...
pub fn parse_field(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, StagedField), CompileErr> {
    let mut checks_ = vec![];
    ...
    while let Some(token) = tokens.get(info.cursor) {
        match token.ttype {
            ...
            TokenType::CalculatedVal => {
                ...
                let calc = expect(
                    tokens,
                    &vec![TokenType::Exclamation, TokenType::Calculation],
                    info,
                )?;
                match calc.ttype {
            ...
            TokenType::Datatype => {
                ...
                dtype_ = match expect(
                    tokens,
                    &vec![TokenType::Text, TokenType::Number, TokenType::Date],
                    info,
                )?
                .ttype
                {
                ...
                while let Some(token) = tokens.get(info.cursor) {
                    match token.ttype {
                        TokenType::Generate => {
                            ...
                            .ttype
                            {
                                TokenType::OnCreation => Some(SerialTrigger::OnCreation),
                                TokenType::OnCommit => Some(SerialTrigger::OnCommit),
                                _ => unreachable!(),
                            };
                        }
                        TokenType::Next => {
                            expect(tokens, &vec![TokenType::Assignment], info)?;
                            next_ = Some(
                                expect(tokens, &vec![TokenType::IntegerLiteral], info)?
                                    .value
                                    .parse::<usize>()
                                    .expect("unable to parse next."),
                            );
                        }
                        TokenType::Increment => {
                            expect(tokens, &vec![TokenType::Assignment], info)?;
                            increment_ = Some(
                                expect(tokens, &vec![TokenType::IntegerLiteral], info)?
                                    .value
                                    .parse::<usize>()
                                    .expect("unable to parse increment."),
                            );
                        }
                        TokenType::Comma => {}
                        TokenType::CloseBrace => {
            ...
            TokenType::NotEmpty => {
                ...
                match expect(tokens, &vec![TokenType::True, TokenType::False], info)?.ttype {
                    TokenType::True => checks_.push(StagedValidationType::NotEmpty),
                    TokenType::False => {}
                    _ => unreachable!(),
                };
            }
            TokenType::Required => {
                expect(tokens, &vec![TokenType::Assignment], info)?;
                match expect(tokens, &vec![TokenType::True, TokenType::False], info)?.ttype {
                    TokenType::True => checks_.push(StagedValidationType::Required),
                    TokenType::False => {}
                    _ => unreachable!(),
                };
            }
            TokenType::Unique => {
                expect(tokens, &vec![TokenType::Assignment], info)?;
                match expect(tokens, &vec![TokenType::True, TokenType::False], info)?.ttype {
                    TokenType::True => checks_.push(StagedValidationType::Unique),
                    TokenType::False => {}
                    _ => unreachable!(),
                };
            }
...
pub fn parse_table(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, StagedTable), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
        .expect("Unable to parse object ID.");
...
pub fn parse_extern(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, DataSource), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
    ...
    if filename_.value.ends_with(".cad") {
        ...
        return Err(CompileErr::UnknownFileType {
            filename: name_.clone(),
        });
    }
}

pub fn parse_table_occurrence(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, StagedOccurrence), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
        .expect("Unable to parse object id.");
...
pub fn parse_script(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, ProtoScript), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
        .expect("Unable to parse object number.");
    let name_ = expect(tokens, &vec![TokenType::Identifier], info)?
        .value
        .clone();
...
pub fn parse_value_list_attributes(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(Option<Token>, StagedValueListSortBy), CompileErr> {
    let mut from_ = None;
    ...
    loop {
        match expect(
            tokens,
            &vec![TokenType::From, TokenType::Sort, TokenType::CloseBrace],
            info,
        )?
        .ttype
        {
            TokenType::From => {
                ...
                if let Some(token) = tokens.get(info.cursor) {
                    match token.ttype {
                        ...
                        TokenType::CloseBrace => return Ok((from_.cloned(), sort_)),
                        _ => {
                            return Err(CompileErr::UnexpectedToken {
                                token: token.clone(),
...
pub fn parse_value_list(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, StagedValueList), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
        .expect("Unable to parse object ID.");
            ...
            TokenType::Comma => {
                ...
                match token.ttype {
                    ...
                    TokenType::Assignment => {
                        expect(tokens, &vec![TokenType::OpenBrace], info)?;
                        (from_, sort_) = parse_value_list_attributes(tokens, info)
                            .expect("Unable to parse value list attributes.");
                        return Ok((
                            id_,
                            StagedValueList {
                                id: id_,
            ...
            TokenType::String => {
                ...
                match token.ttype {
                    ...
                    _ => {
                        return Err(CompileErr::UnexpectedToken {
                            token: token.clone(),
                            expected: vec![TokenType::CloseBrace, TokenType::Comma],
                        })
                    }
                }
            }
...
pub fn parse_relation_criteria<'a>(
    tokens: &'a [Token],
    info: &mut ParseInfo,
) -> Result<StagedRelationCriteria, CompileErr> {
...
pub fn parse_relation(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, StagedRelation), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
        .expect("Unable to parse object id.");
    ...
    let token = expect(
        tokens,
        &vec![TokenType::OpenBrace, TokenType::Identifier],
        info,
    )?;
...
pub fn parse_layout_attributes(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<Vec<LayoutAttribute>, CompileErr> {
    let attributes = vec![];
    while let Some(token) = tokens.get(info.cursor) {
        match token.ttype {
            TokenType::CloseBrace => {
...
pub fn parse_layout(
    tokens: &[Token],
    info: &mut ParseInfo,
) -> Result<(u16, StagedLayout), CompileErr> {
    let id_ = expect(tokens, &vec![TokenType::ObjectNumber], info)?
        .value
        .parse::<u16>()
        .expect("Unable to parse object id.");
    ...
    fn basic_table_parse_test() {
        ...
        expected_fields.insert(
            ...
            StagedField {
                ...
                validation: StagedValidation {
                    checks: vec![StagedValidationType::Required, StagedValidationType::Unique],
                    message: String::new(),
